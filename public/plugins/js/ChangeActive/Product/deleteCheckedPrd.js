        ...
        if ($('.deleteAll').length) {
            $(document).on('click', '.deleteAll', function (e) {
                ...
                Swal.fire({
                    ...
                    text: "Hành động này sẽ xóa tất cả sản phẩm",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'DELETE',
                            url: '/product/ajax/deleteCheckedProduct',
                            data: option,
                            dataType: 'json',
                            success: function (res) {
                                ...
                                if (res.totalCountAfter > 0) {
                                    ...
                                    $('.null_Table').html('<tr><td valign="top" colspan="9" class="dataTables_empty">Không tìm thấy dòng nào phù hợp</td></tr>');
                                    $('.countTrash').html('(' + res.trashedCount + ')');
                                }
                            },
