        ...
        if ($('.deleteAll').length) {
            $(document).on('click', '.deleteAll', function (e) {
                ...
                Swal.fire({
                    ...
                    text: "Hành động này sẽ xóa tất cả bài viết đã chọn",
                    icon: 'warning',
                            ...
                            success: function (res) {
                                if (res.totalCountAfter > 0) {
                                    if (res.status) {
                                        ...
                                        $('.blogCout').html('('+res.trashedCount+')');
                                        HT.recalculateSTT();
                                    }else{
                                    ...
                                    $('.null_Table').html('<tr><td valign="top" colspan="7" class="dataTables_empty">Không tìm thấy dòng nào phù hợp</td></tr>');
                                    $('.blogCout').html('('+res.trashedCount+')');
                                }
                            },
